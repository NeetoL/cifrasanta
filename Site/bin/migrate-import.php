...
try {
 $sql=file_get_contents(dirname(__DIR__).'/database/004_importacao.sql');$checksum=hash('sha256',$sql);
 $q=$db->prepare('SELECT checksum FROM cifra_santa_migracao WHERE versao=?');$q->execute(['004_importacao']);$saved=$q->fetchColumn();
 ...
 if(!$saved) {
  ...
  $db->prepare('INSERT INTO cifra_santa_migracao(versao,checksum) VALUES(?,?)')->execute(['004_importacao',$checksum]);
 }
 echo "Tabela de importação pronta.\n";
