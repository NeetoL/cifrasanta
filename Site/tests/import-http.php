...
$base='http://127.0.0.1:8104';$password=bin2hex(random_bytes(16));
...
try{
 ...
 okImport(str_contains($body,'MÚSICA IDENTIFICADA')&&str_contains($body,'Odres Novos')&&str_contains($body,'Cifra Club')&&str_contains($body,'catálogo de terceiros')&&str_contains($body,'Salvar cifra')&&str_contains($body,'Editar')&&str_contains($body,'Cancelar'),'prévia do caso real');
 ...
 [$status,$body]=httpImport('/admin-import.php',$cookies[0],['action'=>'save','token'=>$real[1]??'','csrf'=>$csrf,'categoria'=>'Entrada','autorizado'=>'1']);okImport($status===422&&str_contains($body,'Cole a cifra'),'catálogo de terceiros não salva sem cifra própria');
